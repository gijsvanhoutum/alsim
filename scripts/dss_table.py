...
def plot(df):
    ...
    num = 10
    fig3 = plt.figure(figsize=mm2inch(90,90))
    
    gs = fig3.add_gridspec(nrows=2, 
                           ncols=3,
                           left=0.125,
                           right=1,
                           top=0.95,
    ...
    ax = [0,0,0]
    ax[0] = fig3.add_subplot(gs[0,:-1])
    ax[0].set_title('(a)',fontsize=8)
    format_ax(ax[0])
    ax[1] = fig3.add_subplot(gs[0,2])
    ax[1].set_title('(b)',fontsize=8)
    format_ax(ax[1])
    ax[2] = fig3.add_subplot(gs[1,:])
    ax[2].set_title('(c)',fontsize=8)
    format_ax(ax[2])
    ...
    for i,(f,dff) in enumerate( df.groupby("f") ):
        ...
        ax[0].plot(x,q50["sc"],color=colors[i],ls="-",lw=1,marker=".",ms=4,label=f)
        # ax[0].plot(x,q5["sc"],color=colors[i],ls="--",lw=0.5)
        # ax[0].plot(x,q95["sc"],color=colors[i],ls="--",lw=0.5)
        ax[0].fill_between(x,q5["sc"],q95["sc"],alpha=0.15,color=colors[i])
                    
    keys = []
    for i,(c,df16) in enumerate( df[ (df["n"]==16) & (df["f"]=="50/50% Grad. Int.")].groupby("cf") ):

        q5 = df16.quantile(q=0.25)["sc"]
        q50 = df16.quantile(q=0.5)["sc"]
        q95 = df16.quantile(q=0.75)["sc"]
    
        keys.append(c)
        ax[1].bar(i, q50, yerr=np.vstack([q50-q5, q95-q50]),capsize=4,color="C0",ecolor="black")
    ...
    for i,c in enumerate( np.unique(total[:,-1]) ):
        ...
        ax[2].plot(x[ids[0]],q50[ids[0]],color=colors[i],ls="-",lw=1,label=label,marker=".",ms=4)
        
        ax[2].fill_between(x[ids[0]],q5[ids[0]],q95[ids[0]],alpha=0.15,color=colors[i])
        ax[2].plot(x[ids[1]],q50[ids[1]],color=colors[i],ls="-",lw=1,marker=".",ms=4)
        ax[2].fill_between(x[ids[1]],q5[ids[1]],q95[ids[1]],alpha=0.15,color=colors[i])
    ...
    ax[0].set_ylim([df["sc"].min(),df["sc"].max()])  
    ...
    ax[1].set_xticklabels(keys,fontsize=6)
    ax[1].set_yticks([])
    ax[1].set_ylim([df["sc"].min(),df["sc"].max()])    
    ax[1].set_xlabel("Classifier",fontsize=6)
    ax[1].spines["left"].set_visible(False)

    
    ax[2].set_xticks(x)
    ax[2].set_xticklabels(x,fontsize=6)    
    ax[2].set_ylim([-0.5,0.5])  
    ax[2].set_yticks([-0.5,0,0.5])
    ax[2].set_yticklabels([-0.5,0,0.5],fontsize=6)   
    leg = ax[2].legend(fontsize=6,
                 frameon=False,
                 loc="lower right",
                 handlelength=1,
                 borderaxespad=0.5,
    ...
    ax[2].set_xlabel("Features",fontsize=6)
    ax[2].set_ylabel("Value",fontsize=6)
    
    secax = ax[2].secondary_xaxis('top')
    secax.set_xticks([0,3.5,7.5,11.5,15])
    secax.set_xticklabels(["   | min","<- gradient ->","max |   | min","<- intensity ->","max |   "],fontsize=6)  

    secax.tick_params(top=False)
...
if __name__=="__main__":
    ...
    source_path = data_dir+r"\source\xiris"
    save_path = "xiris.npz"  
    ...
    fig.savefig(save_dir+'feature_performance.pdf',
                format='pdf', 
                dpi=600, 
                bbox_inches='tight')