...
class US(QueryStrategy):
    ...
    def __init__(self,method="max"):
        self.map = {'max':self.max,'entropy':self.entropy}       
...
class RUS(US):
    ...
    """
    def __init__(self,p=0.5,method="max"):
...
class BEE(US):
    ...
    """
    def __init__(self,l=0.1,e=0.01,method="max"):
        super().__init__(method=method)
...
class EGA(US):
    ...
    Active Learning
    """
    def __init__(self,b=0.1,t=0.1,num=11,method="max"):
        super().__init__(method=method) 
        self.b = b
        self.t = t
...
class UDD(US): 
    ...
    """
    def __init__(self,b=1/3,l=1/3,method="max"):
        super().__init__(method=method)   
        self.b = b
        self.l = l
    ...
    def act(self,state,number=1): 
        ...
        for i in np.arange(number):
            d = self.norm( self.d[:,~unknown][unknown].min(axis=1) )
            h = self.norm( self.h[unknown] )
            f = self.norm( s[unknown] )
            u = (1-self.l-self.b)*f+self.b*h+self.l*d
            w = (u==u.max()).astype(int)
            p = w / np.count_nonzero(w)
            index = np.random.multinomial(1,p).astype(np.bool)
            actions[i] = self.rng[unknown][index] 
            unknown[actions[i]] = False
