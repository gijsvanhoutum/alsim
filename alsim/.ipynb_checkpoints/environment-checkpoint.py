...
class Environment: 
    ...
    def reset(self):
        ...
        self.known = np.ones(len(self.y_trn),dtype=bool)
    ...
    def _train(self): 
        if self.ratio <= 0 or self.ratio >= 1:
            train = test = np.ones(self.y.size,dtype=bool)
        else:
            train = np.zeros(self.y.size,dtype=bool)
            for k in self.uy:
    ...
    def _initial(self, train ):
        known = np.zeros(self.y.size,dtype=bool) 
        for k in self.uy:
